...
public:
   ...
   T const& kesimo(int k){
       if (nelems < k) throw std::domain_error("??");
       return kesimo(k, raiz);
   }

   T const& kesimo(int k, Link r) const {
       if (r == nullptr) throw domain_error("??");
       else if (k == r->tam_i) return r->elem;
       else if (k < r->tam_i){
           return kesimo(k, r->iz);
       }
       else {
           return kesimo(k - r->tam_i, r->dr);
       }
    }
