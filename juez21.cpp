...
void dfs(const Digrafo& G, int v, bool& termina, bool& a_veces, bool& nunca, vector<bool>& marcaje, vector<bool>& apilado){
   marcaje[v] = true;
   apilado[v] = true;
   if (G.ady(v).empty()) {
       termina = true;
       if (nunca) a_veces = true;
       nunca = false;
   }
   for (int w : G.ady(v)){
      if (!marcaje[w]){
         dfs(G, w, termina, a_veces, nunca,  marcaje, apilado); 
      }
      else if (apilado[w]) {
         if (termina) a_veces = true;
         else nunca = true;
      }
   }
   apilado[v] = false;
...
bool resuelveCaso() {
    ...
    dfs(g, 0, termina, a_veces, nunca, marcaje, apilado);
    if (a_veces) cout << "A VECES\n";
    else if (nunca) cout << "NUNCA\n";
    else cout << "SIEMPRE\n";
