...
long long int resolver(PriorityQueue<long long int> mont){
    long long int min = 0;
    while (mont.size() > 1){
        long long int e1 = mont.top();
        mont.pop();
        long long int e2 = mont.top();
        mont.pop();
        long long int sum = e1 + e2;
        min += sum;
        mont.push(sum);
    }
    return min;
...
bool resuelveCaso() {
    ...
    cin >> N;
    if (N == 0) return false;
    PriorityQueue<long long int> mont = PriorityQueue<long long int>();
    long long int n;
    for (int i = 0; i < N; ++i) {
        cin >> n;
        mont.push(n);
