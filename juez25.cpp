...
ConjuntosDisjuntos ARM_kruskal(GrafoValorado<int> const& g, int &coste, vector<Arista<int> >& ARM){
    PriorityQueue<Arista<int> > pq(g.aristas());
    ConjuntosDisjuntos cjutos(g.V());
    while (!pq.empty()){
        auto a = pq.top();
        pq.pop();
        int v = a.uno();
        int w = a.otro(v);
        if (!cjutos.unidos(v, w)){
            cjutos.unir(v, w);
            ARM.push_back(a);
            coste += a.valor();
...
bool resuelveCaso() {
   ...
   for (int i = 0; i < M; ++i){
      int a, b, c;
      cin >> a >> b >> c;
      Arista<int> arista(a - 1, b - 1, c);
      g.ponArista(arista);
   }
   vector<Arista<int> > ARM;
   int coste = 0;
   int aeropuertos = 0;
   ConjuntosDisjuntos cjutos = ARM_kruskal(g, coste, ARM);

   aeropuertos += cjutos.num_cjtos();
   cout << coste + aeropuertos * A << ' ' << aeropuertos << '\n';
