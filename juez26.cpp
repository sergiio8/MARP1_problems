...
ConjuntosDisjuntos ARM_kruskal (GrafoValorado<int> const& g, vector<Arista<int> >& ARM, int& max){
...
bool resuelveCaso() {
   ...
   if (!cin) return false;
   vector<Arista<int> > ARM(g.V());
   int max = 0;
   ConjuntosDisjuntos cjutos = ARM_kruskal(g, ARM, max);
   if (cjutos.num_cjtos() > 1) cout << "Imposible\n";
   else cout << max << '\n';
