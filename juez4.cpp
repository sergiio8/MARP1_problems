...
vector<int> resolver(PriorityQueue<int>& mont, unordered_map<int, set<int> >& m1, unordered_map<int, int>& m2, int k){
    vector<int> sol;
    for (int i = 0; i < k; ++i){
        int e = mont.top();
        sol.push_back(*m1[e].begin());
        m1[e + m2[*m1[e].begin()]].insert(*m1[e].begin());
        mont.push(e + m2[*m1[e].begin()]);
        mont.pop();
        m1[e].erase(*m1[e].begin());
    }
    return sol;  
}
...
bool resuelveCaso() {
    ...
    PriorityQueue<int> mont = PriorityQueue<int>();
    ...
    vector<int> sol = resolver(mont, m1, m2, k);
